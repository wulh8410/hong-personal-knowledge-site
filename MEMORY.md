...
- 2026-09-12：AITDK 对首页报出“59% 内容依赖 JavaScript”和“服务端仅 137 words”，但线上禁用/启用 JavaScript 实测均可读取相同的 1881 个中文字符、32 个标题和 14 个 `article`；确认属于中文分词或 Next.js HTML 识别偏差，不通过隐藏文本或重复堆文案迎合评分。首页可见更新时间改为读取统一 `siteConfig.lastModified`。
...
- 2026-07-08：首页“懂技术，也懂运营”现场图不再依赖 1px 微调，改为轻微放大上移并加顶部纸白遮罩，彻底去掉原图顶部残留文案；关于页能力时间轴卡片改为上下两行统一字号层级，减少主标题和说明文字混乱。
- 2026-07-08：用户反馈顶部纸白遮罩遮住人物头部，已撤销遮罩和额外位移缩放；首页“懂技术，也懂运营”桌面现场图按 `16:5` 正常显示，建议新素材按 `2688×840` 出图，最低不低于 `1344×420`。
...
- 2026-07-10：顶部导航已调整为「首页 / 文章 / 知识库 / 小课堂 / 关于我」，其中「小课堂」链接仍为 `/courses`。从 `资料/课程/微信小店` 导入 8 节本地图文课：一起买、小店会员、推荐场景、搜索场景、电商罗盘、礼物营销、营销优惠券、评价抽奖；对应 Word 文字稿和信息图均已进入 `content/courses` 与 `public/images/courses`。这些资料暂时没有直播回放，frontmatter 的 `replayUrl` 保持为空，前台仅展示图文学习；后续补入链接即可自动显示回放入口。导入脚本为 `scripts/import-local-wechat-store-course-materials.py`，会清理 Markdown 双星号、分隔线和 Word 内嵌表格的无效图片占位。
